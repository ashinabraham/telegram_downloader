...
class TestPathManager:
    ...
    @patch("os.listdir")
    @pytest.mark.asyncio
    async def test_get_directory_options_success(
        self, mock_listdir, mock_exists, mock_isdir
    ):
        ...
        mock_isdir.side_effect = lambda path: any(
            folder in path for folder in ["folder1", "folder2"]
        )
    ...
    def test_multiple_path_encodings(self):
        ...
        path_manager = PathManager()
        paths = ["/path1", "/path2", "/path3", "/path1", "/path4"]  # Duplicate
    ...
    def test_sanitize_filename_edge_cases(self):
        ...
        test_cases = [
            ...
            (
                "file_with_mixed/slashes\\and:colons.txt",
                "file_with_mixed_slashes_and:colons.txt",
            ),
            ("", ""),
            ("   ", ""),
        ...
        for original, expected in test_cases:
            ...
            if result:  # Only check non-empty results
                assert (
                    "/" not in result
                ), f"Forward slashes not removed from '{original}'"
                assert "\\" not in result, f"Backslashes not removed from '{original}'"
                assert (
                    result.strip() == result
                ), f"Leading/trailing spaces not removed from '{original}'"
