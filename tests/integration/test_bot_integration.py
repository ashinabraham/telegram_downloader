...
class TestBotIntegration:
    ...
    async def test_file_download_workflow(self, setup_bot_components, temp_dir):
        ...
        task = await download_manager.queue_download(user_id, file_message, f"{save_dir}/test_file.pdf")
    ...
    @pytest.mark.asyncio
    async def test_directory_navigation_workflow(self, setup_bot_components, sample_directory_structure):
        ...
        # Test path joining
        joined_path = path_manager.join_paths(sample_directory_structure, "folder1", "test_file.txt")
    ...
    async def test_download_manager_operations(self, setup_bot_components):
        ...
        for i, file_message in enumerate(file_messages):
            ...
            task = await download_manager.queue_download(user_id, file_message, f"/test/path/file{i}.txt")
    ...
    async def test_error_handling_integration(self, setup_bot_components):
        ...
        assert user_state.get_user_data(user_id, 'nonexistent', default='default') == 'default'
    ...
    async def test_concurrent_operations(self, setup_bot_components):
        ...
        for i, file_message in enumerate(file_messages):
            ...
            task = await download_manager.queue_download(f"user{i}", file_message, f"/test/path/file{i}.txt")
            assert task.status == "queued"
    ...
    async def test_filename_sanitization_workflow(self, setup_bot_components, temp_dir):
        ...
        for original_filename in problematic_filenames:
            ...
            assert '\\' not in sanitized_filename
            assert sanitized_filename.strip() == sanitized_filename  # No leading/trailing spaces
